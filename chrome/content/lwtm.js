...
var LWTM = {
  ...
  get currentTheme () {
    try {
      if (_prefs.getBoolPref("isThemeSelected"))
        //var data = JSON.parse(_prefs.getCharPref("currentTheme")); // CUSTOM
  ...
  set currentTheme (aData) {
    ...
    // CUSTOM. To Be changed BEFORE we notify Observers.
    _prefs.setCharPref("currentTheme",JSON.stringify(aData));
  ...
  resetPreview: function () {
    if (_previewTimer) {
      _previewTimer.cancel();
      _previewTimer = null;
      _notifyWindows(LightweightThemeManager.currentThemeForDisplay);
    }
  },
...
function _sanitizeTheme(aData, aBaseURI) {
  ...
  function sanitizeProperty(prop) {
    ...
    try {
      ...
      /**
       * 
       * Adjusted, so we can use local images for the personas.
       */
      //if (/^https:/.test(val))
      return val;
